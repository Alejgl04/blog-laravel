    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      @foreach ($posts as $post)
        <article class="w-full h-80 bg-cover bg-center @if ( $loop->first ) md:col-span-2 @endif" 
                 style="background-image: url(@if ( $post->image ) @if (env('APP_ENV')=='local') {{Storage::url( $post->image->url )}} @else {{ $post->image->url }} @endif @else https://cdn.pixabay.com/photo/2022/01/22/09/42/buildings-6956678_960_720.jpg @endif)">
